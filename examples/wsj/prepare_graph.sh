...
if [ $stage -le 3 ]; then
  ...
  utils/lang/check_phones_compatible.sh \
    $langdir/$unit/lang_${unit}_test_bd_tgpr/phones.txt $lang/phones.txt
  utils/mkgraph.sh \
    --self-loop-scale 1.0 $langdir/${unit}/lang_${unit}_test_bd_tgpr \
    $graph $graph/graph_bd_tgpr || exit 1;
fi
